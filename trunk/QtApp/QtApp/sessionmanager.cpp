...
// #define ALERT_TYPESTR_REDIRECT "redirect" 
			...
			if(props[KEY_TYPE]==ALERT_TYPESTR_NOTIFY)
			{
				...
				//对ALERT的确认将影响客户端的最后版本信息！只有全部都能处理的情况，才返回200，也可以根据配置和具体业务的情况返回
...
int SessionManager::do_state(Client *cl,CommandMap state_props,CommandMap cmd_props,QByteArray data)
{
	...
	if(cmd_props["0"]==get_cmdstr(CMD_ALERT))
	{
		if(cmd_props["type"]==ALERT_TYPESTR_SIGNUP)
		{
			...
			{
				emit trayMessage(tr("注册成功"),"你的帐号"+synconf->getstr("user_id")+"成功注册！");
			...
			else
			{
				...
				emit trayMessage(tr("注册失败"),"你的帐号"+synconf->getstr("user_id")+"注册失败！错误代码["+state_props["2"]+"]！原因：["+strReason+"]");
			}
		}
		return ERR_NONE;
	}
	if(cmd_props["0"]==get_cmdstr(CMD_HELLO))
	{
		if(cmd_props["session"]!="")
		{
			if(state_props["2"].toInt()==STA_OK)
			{
				emit trayMessage(tr("登录成功"),"你的帐号"+synconf->getstr("user_id")+"成功登录！");
			...
			{				
				QString strReason=state_props["reason"];
				emit trayMessage(tr("登录失败"),"你的帐号"+synconf->getstr("user_id")+"登录失败！错误代码["+state_props["2"]+"]！原因：["+strReason+"]");
			}
		}
		else
		{
			QString strReason=state_props["reason"];
			emit trayMessage(tr("连接成功"),"你成功的连接到服务器，但是尚未登录！");
		}
		return ERR_NONE;
	}
